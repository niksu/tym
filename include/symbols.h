...
struct term_database_t {
  struct Terms * herbrand_universe;
  struct Terms * term_database[TERM_DATABASE_SIZE];
};

struct term_database_t * mk_term_database(void);
bool term_database_add(struct Term * term, struct term_database_t * tdb);
struct buffer_write_result * term_database_str(struct term_database_t * tdb, struct buffer_info * dst);

struct predicate_t {
  const char * predicate;
  struct Clauses * bodies;
  uint8_t arity;
};
...
bool atom_database_member(const struct Atom * atom, struct atom_database_t * adb, enum adl_lookup_error * error_code, struct predicate_t ** record);

enum adl_add_error {NO_ATOM_DATABASE = 0};

bool atom_database_add(const struct Atom * atom, struct atom_database_t * adb, enum adl_add_error * error_code, struct predicate_t ** result);
...
bool clause_database_add(struct Clause * clause, struct atom_database_t * cdb, enum cdl_add_error *);
